...
const ParticleField = ({ count = 2000, theme = 'glass_universe' }) => {
  ...
  const themeParticleColors = {
    ...
    // Space Collection
    galaxy_core: ['#6c63ff', '#00d4ff', '#ffffff'],
    warp_speed: ['#ffffff', '#ffffff'],
    black_hole: ['#ff5e00', '#ff0000', '#000'],
    nebula: ['#3c00ff', '#ff00ff', '#ffffff'],
  ...
  useFrame((state) => {
    ...
    if (points.current && theme !== 'warp_speed') {
  ...
  return (
    <points ref={points}>
      ...
      <pointsMaterial size={0.03} vertexColors transparent opacity={0.4} blending={THREE.AdditiveBlending} />
    </points>
  );
};
